...
Metadata for turning Flows, Validation Rules and Apex off without deactivating each component one at a time. For example, you can switch them off for a data load or migration.
...
The switch only affects components that check it. A Flow, Validation Rule or Apex class that does not reference it keeps running whatever it is set to. Any new automation must include the check to be covered.

It does not affect Workflow Rules, Process Builder, duplicate rules, required fields, lookup filters or managed-package components.
...
3. Limit who can change the custom setting. Anyone with the **Customize Application** permission can edit it.
