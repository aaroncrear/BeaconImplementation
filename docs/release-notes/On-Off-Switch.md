...
6. To confirm a Validation Rule honors the switch and the User field, create a test Validation Rule in a sandbox with the formula `$Setup.On_Off_Switch__c.Run_Validation_Rules__c && NOT($User.Bypass_Val_Rules__c) && ISBLANK(Description)` on an object of your choice.
...
7. To confirm a record-triggered Flow honors the switch and the User field, add `{!$Setup.On_Off_Switch__c.Run_Flows__c} && NOT({!$User.Bypass_Flows__c})` (Formula Evaluates to True) to a test Flow's entry conditions in a sandbox.
...
8. Confirm both only affect the intended user: with the Org Default all checked, and one user's User level On/Off Switch record or bypass fields set to bypass, confirm a second user still gets the Validation Rule error and the Flow still runs.
9. Remove the test Validation Rule and Flow after testing.
...
- Grant **Edit** access to the three User bypass fields to admins only, through field-level security or a permission set. Keep other users at read-only or no access, so they can't bypass automation for themselves. Add the fields to the **User Layout** if admins should set them from the user's page.
