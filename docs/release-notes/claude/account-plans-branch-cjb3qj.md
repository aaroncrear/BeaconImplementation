...
- Added `standard-ActionPlan` and `standard-ActionPlanTemplate` tab visibility (`Visible`) to all eight permission sets. `AccountPlan`, `AccountPlanObjective`, `AccountPlanObjectiveMeasure`, and `AccountPlanObjMeasRela` do not have dedicated app tabs in this org (they're surfaced via Account related lists), but `ActionPlan` and `ActionPlanTemplate` do ship with standard tabs, and object/field permissions alone are not sufficient for users to navigate to them without the tab also being made visible.
...
| 22 | Permission Set | Action Plan | Beacon_Consulting_Object_Tab_FLS, Beacon_Customer_Success_Object_Tab_FLS, Beacon_Sales_Object_Tab_FLS, Beacon_Executive_Object_Tab_FLS, Beacon_Marketing_Object_Tab_FLS, Beacon_Product_Object_Tab_FLS, Beacon_ResOps_Object_Tab_FLS, Beacon_Tech_Object_Tab_FLS | standard-ActionPlan tab | Updated | Set tab visibility to Visible so users can navigate to Action Plans. |
| 23 | Permission Set | Action Plan Template | Beacon_Consulting_Object_Tab_FLS, Beacon_Customer_Success_Object_Tab_FLS, Beacon_Sales_Object_Tab_FLS, Beacon_Executive_Object_Tab_FLS, Beacon_Marketing_Object_Tab_FLS, Beacon_Product_Object_Tab_FLS, Beacon_ResOps_Object_Tab_FLS, Beacon_Tech_Object_Tab_FLS | standard-ActionPlanTemplate tab | Updated | Set tab visibility to Visible so users can navigate to Action Plan Templates. |
